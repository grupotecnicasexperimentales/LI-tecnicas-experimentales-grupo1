...
def calculate_spectral_quality(frame):
    """
    Calcula la calidad de la imagen basándose en la energía de alta frecuencia usando la Transformada de Fourier (FFT).
    """
...
def rank_frames(video_frames, method="combined"):
    ...
    for i in range(num_frames):
        ...
        if method == "peak":
            ...
            # Combinación de espectral y gradiente
...
def load_fits_frames(folder_path):
    ...
    if not frames:
        raise ValueError("No se pudieron cargar datos válidos.") #debuggin
...
def align_frames_standard(frames, reference):
    """
    Alinea los frames respecto a la referencia usando correlación cruzada (FFT).
    """
    ...
    for i in range(num_frames):
        ...
        # Registro por correlación cruzada usando FFT
...
def align_frames_robust(frames, reference, sigma_smooth=0):
    ...
    for i in range(num_frames):
        ...
        # Correlación usando las versiones suavizadas
...
def compute_shift(ref, img):
    """Calcula el desplazamiento (y, x) usando correlación cruzada FFT."""
...
def apply_safe_deconvolution(image):
    """
    Aplica deconvolución Lucy-Richardson con PSF Gaussiana.
    """
...
if __name__ == "__main__":
    ...
    frames_folder = "/work/alex.aguilera/Light"
